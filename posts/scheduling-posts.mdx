...
- All channels share the same scheduled time by default
- Each channel publishes separately and has its own status
- One channel can succeed while another fails
- You can retry failed channels without affecting successful ones

### Selecting which channels to schedule
...
### Adding channels after scheduling

<Warning>
  If you add cross-post channels to an already-scheduled post, the new channels are **not automatically scheduled**. You'll need to click **Edit scheduling** in the sidebar to add them to the schedule.
</Warning>

View per-channel status by hovering over each platform icon in the post or on the calendar.
...
Your workspace timezone is configured in **Settings → General**. This is the timezone used for:
...
<AccordionGroup>
  <Accordion title="Post published at wrong time">
    Times display in your local timezone but publish according to the workspace timezone. Hover over the scheduled time to verify the workspace timezone conversion is what you expect.
  </Accordion>
