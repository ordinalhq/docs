...
### One-click approvals from Slack

Approval request DMs include an **Approve** button so reviewers can sign off without opening Ordinal. Clicking **Approve** records the approval, notifies the requester, and updates the Slack message with a **View post** link. Only the assigned approver's Slack account can use the button. See [Approving from Slack](/collaboration/approvals#approving-from-slack) for details.
