---
title: "Notifications"
description: "Choose which Ordinal events trigger notifications and whether they go to in-app, email, or Slack."
keywords: ["Ordinal notifications", "social media notifications", "approval notifications"]
icon: "bell"
---
...
When someone connects Slack notifications for the first time, the Slack workspace is linked to Ordinal so teammates whose email matches a Slack user can receive DMs. Teammates who later accept an invite are connected automatically when their email matches a Slack member.

To stop notifications for everyone in a connected Slack workspace:

1. Go to [Integrations → Slack](https://app.tryordinal.com/settings/integrations/slack)
2. Find the workspace under **Slack Notifications**
3. Open the **…** menu and select **Disconnect Workspace**
4. Confirm in the dialog

After disconnecting, no one in the Ordinal workspace receives Slack DMs from that Slack workspace. Reconnecting Slack notifications creates a fresh link - the person who reconnects is shown as "Connected by," and matched teammates are reconnected automatically.

<Note>
  Disconnecting a workspace only stops Slack DM notifications. It does not affect [Slack Boost](/posts/slack-boost) channels or [scheduled Slack messages](/social-profiles/slack), which are managed separately.
</Note>
