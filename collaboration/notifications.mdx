...
### Agency default Slack workspace

If you manage multiple client workspaces, connect a default Slack workspace at the agency level so every new client workspace inherits the connection automatically. Team members in those workspaces receive their personal Ordinal notifications in the agency Slack, matched by email.

**To set up:**

1. Go to **Agency Settings → Slack Notifications** in the sidebar
2. Click **Connect** and authorize your agency Slack workspace
3. New client workspaces you create from then on inherit this connection

You can connect more than one Slack workspace if your team is split across them - Ordinal routes each user's notifications to the workspace that matches their email.

<Note>
  Only agency accounts can access this page. Existing client workspaces keep any per-workspace Slack connection they already had; the agency default applies to workspaces created after the connection is set up.
</Note>
