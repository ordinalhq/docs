...
<CardGroup cols={2}>
  <Card title="Agencies" icon="users">
    Get client sign-off before publishing to their accounts.
  </Card>
  <Card title="Executive content" icon="user-tie">
    Ensure leadership reviews posts on their personal brands.
  </Card>
  <Card title="Brand consistency" icon="palette">
    Maintain voice and messaging standards across teams.
  </Card>
  <Card title="Compliance" icon="shield-check">
    Meet regulatory requirements with documented approval trails.
  </Card>
</CardGroup>
...
4. **Publish** — Once approved, the post can be scheduled and published
...
3. **If all looks good - **a**pprove** — Click the checkmark. The requester is notified that the post is approved.

<Note>
  Approvals work on mobile too. Review and approve posts from anywhere.
</Note>

## Default Blocking Approvals

For accounts that should **always** require approval, set up automatic rules:

1. Go to **Settings → Approval Rules**
2. Click **Add Rule**
3. Select the account (e.g., CEO's LinkedIn, Client's Twitter)
4. Choose who must approve
5. Save

Any post to that account automatically requires approval from the specified reviewers. Rules are always blocking—posts can't be scheduled without approval.

<Note>
  **Enterprise feature:** On Enterprise plans, you can configure default approval rules that apply workspace-wide. This ensures certain accounts never publish without review—even if someone forgets to request approval manually.
</Note>
