---
title: "Approvals"
description: "Add reviewers to a post, set blocking or non-blocking approval, and require sign-off before content publishes."
keywords: ["social media approvals", "Ordinal approvals", "post approval workflow", "content approval"]
icon: "circle-check"
---
...
The **Approvals** page in the sidebar gives you a centralized view of posts still awaiting your approval, as well as outstanding approvals you've requested. The inbox is split into two tabs:
...
Each tab is reflected in the page URL, so you can bookmark or share a link to a specific tab.

### Filtering the inbox

By default, the inbox shows **Posts & Ideas** with a publish date in the **last 14 days**. Ideas always appear regardless of publish date because they aren't scheduled yet.

- **Publish date** - Pick a preset (Last 14 days, This week, Next 14 days) or a custom range. Only posts are filtered by date.
- **Content type** - Toggle between Posts, Ideas, or both.

Click a column header to sort either table, including the **Requested** column to order approvals by when they were requested.

<Note>
  The Approvals badge in the sidebar uses the same default filters: it counts all pending ideas plus posts publishing in the last 14 days, so it matches what you see when you open the inbox.
</Note>
