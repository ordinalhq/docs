...
description: "Start a new post from the calendar, the P keyboard shortcut, an existing idea, or by duplicating another post."
icon: "plus"
---
...
| **Account**        | Select the connected account the post should go to. When a channel has multiple connected profiles, a nested submenu opens where you can pick a specific profile. Search the dropdown by channel, profile name, or handle. You can add cross-post channels later. |
...
:::tip
If you frequently post to the same account, set it as your default in [Post Defaults](/posts/post-defaults) to skip selecting it each time. When a channel has a default profile, the account picker pre-selects that profile by name instead of the platform.
:::
