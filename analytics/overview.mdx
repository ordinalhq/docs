...
<Note>
  Video metrics only populate for posts with type **Video**. Non-video posts show `-` in these columns. Personal profile video stats require the `r_member_postAnalytics` LinkedIn OAuth scope, which is granted when you connect a personal LinkedIn profile.
</Note>
